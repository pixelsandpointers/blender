...
static void give_parvert(const Object *par, int nr, float vec[3], const bool use_evaluated_geometry)
{
  zero_v3(vec);
  ...
  if (par->type == OB_MESH) {
    ...
    if (mesh_eval) {
      ...
      else if (CustomData_has_layer(&mesh_eval->vert_data, CD_ORIGINDEX) && use_evaluated_geometry) {
...
static void ob_parvert3(const Object *ob, const Object *par, float r_mat[4][4])
{
  ...
  if (OB_TYPE_SUPPORT_PARVERT(par->type)) {
    ...
    const bool use_evaluated_geometry = !(ob->transflag & OB_PARENT_USE_FINAL_GEOMETRY);

    give_parvert(par, ob->par1, v1, use_evaluated_geometry);
    give_parvert(par, ob->par2, v2, use_evaluated_geometry);
    give_parvert(par, ob->par3, v3, use_evaluated_geometry);
...
void BKE_object_get_parent_matrix(const Object *ob, Object *par, float r_parentmat[4][4])
{
  ...
  switch (ob->partype & PARTYPE) {
    ...
    case PARVERT1:
      unit_m4(r_parentmat);
      give_parvert(par, ob->par1, vec, use_evaluated_geometry);
